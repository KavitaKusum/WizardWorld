...
@HiltViewModel
class HouseDetailsViewModel @Inject constructor(private val houseUseCase: HouseDetailUseCase): ViewModel() {
    ...
    fun getTraitsList(traits: List<Traits>): List<String> {
    ...
    fun getHeadsList(heads: List<Heads>): List<String> {
        val list= mutableListOf<String>()
        for(item in heads)
            list.add(item.name)
        return list
