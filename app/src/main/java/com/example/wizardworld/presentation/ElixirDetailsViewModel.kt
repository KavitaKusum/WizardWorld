...
@HiltViewModel
class ElixirDetailsViewModel @Inject constructor (private val elixirUseCase: ElixirDetailUseCase) : ViewModel() {
    ...
    fun getElixirDetails(elixirId:String){
        viewModelScope.launch {
            elixirUseCase.invoke(elixirId).collect {
                when (it) {
                    is Result.Error -> _viewState.value= ViewState(isLoading = false, data=null, error = it.msg)
                    is Result.Success -> _viewState.value= ViewState(isLoading = false, data=it.result, error = null)
    ...
    fun getIngredientsList(ingredients: List<Ingredient>): List<String> {
    ...
    fun getInventorsList(inventors: List<Inventor>): List<String> {
        val list = mutableListOf<String>()
